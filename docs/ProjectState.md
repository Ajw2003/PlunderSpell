...
| M3 — Open the other Ages | Scaffold only; Bronze and Late Medieval castle art built | 🟡 `HistoricalEra` enum + plumbing only. Castle art: the Bronze Age and Late Medieval sets are built — 25 rooms and wall pieces plus 4 door plugs each, every piece modelled to a reference sheet ([`BronzeAge.md`](art/rooms/BronzeAge.md), [`LateMedieval.md`](art/rooms/LateMedieval.md)); the Age of Powder is next ([`docs/plans/era-castle-rooms.md`](plans/era-castle-rooms.md)). None of it is wired into the generator yet | ❌ — see below, the data model can't produce era-specific content yet |
