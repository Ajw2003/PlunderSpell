# The High Medieval — handoff sheet

*Stratum II · c. 1250 · Stone keeps and reliquaries*

> Generated by `Tools/ArtBible/build_art_bible.py` from `docs/art/data/high.json`. Edit the JSON, not this file. Rules every entry obeys: [`BRIEF.md`](BRIEF.md). Scale: [`docs/4-systems/scale.md`](../4-systems/scale.md).

My own century, and the one you will learn on — so mind it. Curtain walls of honest limestone, spiral stairs that turn clockwise (not in your favour), and a chapel worth more than everything around it put together. The garrison is small and well-dressed in steel; the household wakes by degrees, a lantern here, a hound there, a knight last of all — which is precisely the order in which you will come to regret waking them.

## Age palette

| Swatch | Hex | Used for |
|---|---|---|
| Limestone ashlar | `#A89F8A` | curtain walls, vaults, stair drums, altar mensa |
| Oak | `#5E4630` | doors, portcullis timbers, drawbars, panel backs, hafts |
| Mail steel | `#7C8288` | hauberks, coifs, chausses — the garrison's grey |
| Undyed wool | `#C4B89C` | gambesons, the argent of the household arms, linen bags |
| Woad blue | `#3E5470` | the household's azure field — surcoats, hound coats, enamel, glass; kermes red #7E2A26 is its second tincture |
| Soot | `#1E1B17` | torch-smoke on vaults, crevice dirt, belly grime |

## Contents

- **Structures:** [The Gatehouse](#gatehouse-portcullis) · [The Newel Stair](#spiral-stair) · [The Chapel](#castle-chapel)
- **Enemies:** [Lantern Warden](#lantern-warden) · [Castle Crossbowman](#castle-crossbowman) · [Household Knight](#household-knight) · [Alaunt War-hound](#alaunt-hound)
- **Plunder:** [Arm Reliquary](#arm-reliquary) · [Silver Ewer](#silver-ewer) · [Illuminated Psalter](#illuminated-psalter) · [Coin Coffer](#coin-coffer) · [Gilded Altarpiece](#gilded-altarpiece)

---

## Structures

<a id="gatehouse-portcullis"></a>
### The Gatehouse

![The Gatehouse concept sheet](concept/high/gatehouse-portcullis.png)

*Twin-towered gate passage with a portcullis, murder-holes in the vault and a barred pair of doors.*

| Zone | Footprint | Height | Budget | Concept |
|---|---|---|---|---|
| CurtainWall (clear 5.20 m) | 12 × 12 m, one cell; block 9.0 × 10.0 m with two D-towers, curtain stubs east and west | 5.20 m | ≤ 25k tris for the cell (towers 9k, passage + vault 6k, portcullis 2k, doors + drawbar 2k, windlass 1k, parapet 3k); limestone tiling set + one oak/iron trim sheet (2048²). | [SVG](concept/high/gatehouse-portcullis.svg) · [PNG](concept/high/gatehouse-portcullis.png) |

Here is the castle's one front door, and it has been designed by a man who assumed you would be rude enough to use it. Two D-shaped towers crowd a vaulted passage; a portcullis waits in its groove, three murder-holes squint down from the vault, and a pair of oak doors take a drawbar the size of a felled tree. It is also your way home: the extraction exit sits in this cell, so every raid ends by walking back under that portcullis and trusting nobody upstairs says TONITRUS.
...
| Material | Hex | Notes |
|---|---|---|
| Limestone ashlar | `#A89F8A` | Tiling 2.4 m (4 × 8 blocks), running bond; rough-honed, mid roughness 0.75; arris chipping below 2 m; lichen-free (it is inside the wall). |
| Rubble core | `#7D7566` | Only on cut faces and the vault mass: lime-mortared rubble, tiling 1.5 m, roughness 0.9. |
| Oak | `#5E4630` | Portcullis, doors, drawbar, windlass: open grain along the length, 1 m tiling on a trim sheet; greasy dark wear on the drawbar ends. |
| Wrought iron | `#3A3632` | Straps, hinges, grilles, portcullis shoes: blackened, roughness 0.55, rust bloom (#7A4A2A) at every nail. |
| Soot | `#1E1B17` | Decal set: torch plume up the vault, murder-hole rims, a sooty band at 2.5–3.3 m along the passage. |
| Torch fire | `#C4542E` | Emissive only: the sconce flame and its warm bounce; a fire cue, never a surface colour. |
...
![The Newel Stair concept sheet](concept/high/spiral-stair.png)

*A clockwise newel stair in a stone drum, linking one floor to the next — built for a right-handed defender coming down.*

| Zone | Footprint | Height | Budget | Concept |
|---|---|---|---|---|
| Keep (clear 4.60 m) | 12 × 12 m, one cell; stair drum Ø 4.0 m outside / 2.4 m inside in the north-east corner, lobby room around it | 4.60 m | ≤ 25k tris for the cell (drum + 26 treads + newel 12k, lobby shell 8k, door + rope 2k); tread/newel trim sheet 2048², limestone tiling set shared with the gatehouse. | [SVG](concept/high/spiral-stair.svg) · [PNG](concept/high/spiral-stair.png) |

Behold the stair that turns clockwise, as every good castle stair does, so that the man above has his sword arm free and you have yours jammed against a pillar. It is a stone drum in the corner of a lobby, twenty-six worn treads around a single newel, with slits for light and a rope for your hand. It links one floor to the next, and it is where the altarpiece will be dropped, because it is always dropped on the stairs.
...
| Material | Hex | Notes |
|---|---|---|
| Limestone ashlar | `#A89F8A` | Drum and lobby walls: curved-block variant 0.45 × 0.23 m on the drum (tiling around the circumference ×28), roughness 0.8. |
| Worn tread | `#B5AC96` | Treads and newel: paler, smoother limestone, roughness 0.45 in the foot-dip, 0.75 elsewhere; polish mask along the walking line 0.3–0.9 m from the newel. |
| Oak | `#5E4630` | Drum door: vertical boards, trim-sheet grain; darkened at hand height. |
| Iron | `#3A3632` | Rope eyes, door straps, slit bars: blackened, rust at the wall entry. |
| Rope rail | `#C4B89C` | Hemp: tube mesh with a twisted-strand normal, grubby darker band where hands slide. |
| Soot | `#1E1B17` | Plume decal over the niche, soffit streaks, crevice AO boost in the tread joints. |
...
<a id="castle-chapel"></a>
### The Chapel

![The Chapel concept sheet](concept/high/castle-chapel.png)

*A small two-bay vaulted chapel with a painted altar, a gilded retable and five stained-glass lancets.*

| Zone | Footprint | Height | Budget | Concept |
|---|---|---|---|---|
| Keep (clear 4.60 m) | 12 × 12 m, one cell; nave 8.4 × 5.4 m inside, sacristy strip to the north | 4.60 m | ≤ 25k tris for the cell (shell + vault 12k, lancets 3k, altar + step 2k, benches 4k, candle stands 1k, sacristy 3k); limestone set shared, one glass atlas 1024², one oak trim 2048². | [SVG](concept/high/castle-chapel.svg) · [PNG](concept/high/castle-chapel.png) |

Here, friends, is where the money is kept, though the priest would call it something else. A two-bay rib-vaulted room of limewashed stone, an altar dressed in red, the gilded retable on top, candles either side, and five windows of coloured glass that will announce your visit to the entire household the moment anything is thrown near them. It is the richest room in the Age, and the most fragile.
...
| Material | Hex | Notes |
|---|---|---|
| Limestone | `#A89F8A` | Ribs, mensa, step, window reveals: honed, roughness 0.65, tiling 2.4 m. |
| Limewash | `#CFC5AE` | Wall and web plaster over ashlar: matte 0.9 roughness, painted joint lines, candle-soot gradient toward the vault crown. |
| Ruby glass | `#7E2A26` | Stained glass: translucent, emissive at night from inside candles, 0.22 m pane grid; hairline crack decals on break state. |
| Woad glass | `#3E5470` | Stained glass, blue panes and roundel grounds; same shader as the ruby. |
| Lead came | `#2A2622` | Cames, saddle bars, candle prickets: dull, roughness 0.6. |
| Oak | `#5E4630` | Benches, sacristy door and chest: trim sheet; seat tops polished. |
| Gilt (plunder) | `#C9A227` | Only on things you can steal: the retable and the vault bosses (bosses are not stealable, so keep them to a rubbed gilt trace on stone). |
...
<a id="lantern-warden"></a>
### Lantern Warden — patrol

![Lantern Warden concept sheet](concept/high/lantern-warden.png)

*The night watch: a quilted man with a horn lantern and a glaive, walking a fixed round.*

| Height | Posted to | Tightest clear height | Budget | Concept |
|---|---|---|---|---|
| 1.80 m (1.00 × standard human) | CurtainWall, OuterBailey | 3.60 m | ≤ 8k tris (body 5.5k, glaive 0.8k, lantern 0.9k, hat 0.5k), one 2048² set (Albedo, Normal, packed ORM) + lantern emissive mask. | [SVG](concept/high/lantern-warden.svg) · [PNG](concept/high/lantern-warden.png) |

The humblest man in the household and the most useful, for he carries the only light on the walls. A padded wool coat, an iron kettle hat, a glaive over one shoulder and a horn lantern swinging in the other hand — you will see the lantern long before you see him, which is the whole point of him and your only advantage. He walks a round; if the lantern stops moving, he has heard something.
...
| Material | Hex | Notes |
|---|---|---|
| Gambeson wool | `#C4B89C` | Undyed wool, quilting channels in the normal map, roughness 0.9; grime gradient to the hem; 0.5 m tiling on a cloth trim. |
| Woad hose | `#3E5470` | Wool knit, mud to the knee (darken 30 %), faint pilling noise. |
| Leather | `#5A4030` | Belt, purse, sheath, shoes: roughness 0.6, cracks at bends, edges burnished lighter. |
| Blackened iron | `#6F7479` | Kettle hat, glaive head, lantern frame: dark oxide with bright edges (brim rim, blade edge) and soot on the undersides. |
| Ash haft | `#6B5238` | Straight grain; hand-polished band at 1.0–1.3 m. |
| Horn pane | `#BFA57A` | Translucent, emissive when lit (#C4542E flame at the core); the flame and its bounce are the only madder on him. |
| Skin | `#A07A5C` | Weathered, stubble on the jaw, cold-red nose tip; face read mostly by lantern under-light. |
...
<a id="castle-crossbowman"></a>
### Castle Crossbowman — ranged

![Castle Crossbowman concept sheet](concept/high/castle-crossbowman.png)

*A hired marksman in mail coif and red padded jack, with a stirrup crossbow that takes an age to span.*

| Height | Posted to | Tightest clear height | Budget | Concept |
|---|---|---|---|---|
| 1.78 m (0.99 × standard human) | CurtainWall, InnerWard | 4.00 m | ≤ 8k tris (body 5.5k, crossbow 1.2k, quiver + bolts 0.7k, cap 0.3k), one 2048² set. | [SVG](concept/high/castle-crossbowman.svg) · [PNG](concept/high/castle-crossbowman.png) |

A professional, which is to say a man paid by the bolt and careful with every one. He wears a mail coif under an iron skull cap, a red quilted jack in the household's colour, and carries a stirrup crossbow that will put a bolt through a door. It then takes him six long seconds, a foot in the stirrup and a belt hook, to span it again — which is when you run at him.
...
| Material | Hex | Notes |
|---|---|---|
| Mail steel | `#7C8288` | Riveted mail: ring pattern in normal + opacity-free albedo, tiling 8 mm rings, roughness 0.45, rust bloom in folds. |
| Kermes jack | `#7E2A26` | Quilted wool, 45° diamond channels in the normal map; sun-faded lighter on the shoulders; not madder — a deeper livery red. |
| Hose wool | `#6A5A45` | Brown wool, mud to the shin. |
| Leather | `#5A4030` | Belt, quiver, boots, prod bindings: roughness 0.6, cracked at the quiver mouth. |
| Oak stock | `#5E4630` | Tiller: fine grain, oiled, roughness 0.4; grease stain around the nut. |
| Horn prod | `#A89878` | Composite horn laminate: banded pale-dark stripes along the length, lacquered, roughness 0.35. |
| Skin | `#A07A5C` | Face only, shaved; squint lines. |
...
<a id="household-knight"></a>
### Household Knight — heavy

![Household Knight concept sheet](concept/high/household-knight.png)

*The lord's own knight in full mail, great helm and the household's arms — the one you go around.*

| Height | Posted to | Tightest clear height | Budget | Concept |
|---|---|---|---|---|
| 1.95 m (1.08 × standard human) | Keep, InnerWard | 4.00 m | ≤ 12k tris (body + mail 7k, helm + crest 1.5k, shield 1k, sword/mace 0.8k, surcoat 1.7k), one 2048² set + a shared mail detail tiling. | [SVG](concept/high/household-knight.svg) · [PNG](concept/high/household-knight.png) |

And here, at last, is the gentleman the whole household is dressed to imitate: mail from crown to toe, a flat-topped great helm with a painted crest, the lord's arms on his surcoat and his shield. He is slow to wake and slower to turn, but he does not stop, and he does not care what you throw at him. Everything else in this castle can be outwitted; him, you avoid.
...
| Material | Hex | Notes |
|---|---|---|
| Mail steel | `#7C8288` | Riveted mail tiling 8 mm, roughness 0.45, rust bloom (darken + warm) in folds and armpits. |
| Helm iron | `#6F7479` | Plate: dark oxide with polished highlights on the brow and crown edge; rivets bright. |
| Woad field | `#3E5470` | Surcoat and shield field: wool (cloth normal) and gesso (smooth) versions; faded on the shoulders. |
| Wool argent | `#C4B89C` | The chevron and crest ribs; soiled at the hem. |
| Kermes gules | `#7E2A26` | Bordure, crest paint; livery red, not madder. |
| Leather | `#5A4030` | Belt, scabbard, enarmes, muffler palms. |
| Limewood | `#5E4630` | Shield core showing at chips and edges; the crest's wooden core. |
...
<a id="alaunt-hound"></a>
### Alaunt War-hound — special

![Alaunt War-hound concept sheet](concept/high/alaunt-hound.png)

*A heavy-jawed fawn war-dog in a spiked collar and the household's padded coat, which hears you first.*

| Height | Posted to | Tightest clear height | Budget | Concept |
|---|---|---|---|---|
| 0.85 m (0.47 × standard human) | OuterBailey, InnerWard | 3.60 m | ≤ 6k tris (dog 4.6k, coat 0.8k, collar + spikes 0.6k), one 2048² set with a fur-direction normal. | [SVG](concept/high/alaunt-hound.svg) · [PNG](concept/high/alaunt-hound.png) |

I have been bitten by a great many things, and I tell you plainly that the alaunt is the worst of them: a broad-skulled hunting dog bred to hold a boar, wearing a spiked collar and a quilted coat in the lord's colours. It sleeps in the bailey and it wakes to noise — any noise — and it does not stop to think about it. It is faster than you, it knows the stairs, and it will have your ankle before the knight has found his helm.
...
| Material | Hex | Notes |
|---|---|---|
| Fawn coat | `#9C7B55` | Short hair: fur-direction normal, faint brindle stripes (#3A3026 at 30 %), roughness 0.8; grime gradient to the belly. |
| Dark mask | `#3A3026` | Muzzle, ears, nose (wet nose roughness 0.2); mask blends into fawn behind the eyes. |
| Collar leather | `#5A4030` | Collar, breast strap, girth: cracked, stiff. |
| Spike iron | `#6F7479` | Collar spikes and ring: dark, sharp bright tips. |
| Woad coat | `#3E5470` | Quilted wool, 5 cm channels; faded at the spine. |
| Wool argent | `#C4B89C` | The chevron on the coat. |
| Kermes gules | `#7E2A26` | The coat's bordure. |
...
<a id="arm-reliquary"></a>
### Arm Reliquary — artifact

![Arm Reliquary concept sheet](concept/high/arm-reliquary.png)

*A silver-gilt forearm raised in blessing, with a rock-crystal window onto a saint's bone.*

| Worth | Bulk | Fragility | Carry | Size | Budget | Concept |
|---|---|---|---|---|---|---|
| 600 coin | 1 st | shatters above 3 m/s impact | one player | 0.16 × 0.14 × 0.52 m (W × D × H) | ≤ 1.5k tris, 1024² set (Albedo, Normal, packed ORM) + crystal refraction mask. | [SVG](concept/high/arm-reliquary.svg) · [PNG](concept/high/arm-reliquary.png) |

Somebody's saint is in there, and somebody's lord paid a great deal to keep him in silver. A forearm and hand in chased silver-gilt, two fingers raised in blessing, standing on a stepped plinth, with a little oval of rock crystal through which you may inspect the relic — and through which, if you drop it, the whole enterprise will come apart. It is the second most valuable thing in the chapel and by far the easiest to carry off.
...
| Material | Hex | Notes |
|---|---|---|
| Silver | `#B8B4A8` | Metallic 1.0, roughness 0.25 on high points, 0.5 in folds; tarnish AO multiplier ×1.5 in grooves. |
| Gilt (orpiment) | `#C9A227` | Metallic 1.0, roughness 0.3; the only gold in the item — plinth, bands, cuff, collet; rubbed off at the corners. |
| Rock crystal | `#CBD3CF` | Transparent, refraction 1.54, faint internal fractures; the fragile part. |
| Garnet | `#6E1F2A` | Cabochons: glossy, roughness 0.1, deep red with a bright spec dot. |
| Relic bone | `#DCD2BA` | Seen only through the window: matte bone on red silk. |
| Niello / soot | `#2A2622` | Black fill in the grooves and plinth mouldings. |

**Grab points.** One hand around the sleeve's middle (0.15–0.30 m up), or both hands on the plinth; a thrown reliquary flies window-first.

**When it breaks.** At 3 m/s the crystal window stars (damage state, worth 450); a second impact at 3 m/s shatters it and the two raised fingers snap at the knuckle — 3 pieces (arm, two-finger piece, the bone), worth drops to 150 on the arm alone, the finger piece 50. The relic bone is a 0-worth prop that the household will be very upset about.

**`LootItem` asset values**

`Worth = 600`, `Bulk = 1`, `Fragility = 3`, `IsArtifact = true`, `DisplayName = "Arm Reliquary"`
...
![Silver Ewer concept sheet](concept/high/silver-ewer.png)

*A hammered silver ewer with a long spout, hinged lid and the household's enamel medallion; it dents, it never breaks.*

| Worth | Bulk | Fragility | Carry | Size | Budget | Concept |
|---|---|---|---|---|---|---|
| 180 coin | 2 st | unbreakable | one player | 0.22 × 0.16 × 0.34 m (W × D × H) | ≤ 1.5k tris, 1024² set; 2 blend shapes for the dent states. | [SVG](concept/high/silver-ewer.svg) · [PNG](concept/high/silver-ewer.png) |

Two stone of silver that sits nicely under one arm and pours wine for the lord at table. It has a long elegant spout, a hinged lid with a gilt knob, a band of black niello, and a small enamel of the household arms in case anyone forgets whose it is. It will not break — silver does not break — but it dents, and every dent is a small, permanent confession.
...
| Material | Hex | Notes |
|---|---|---|
| Silver | `#B8B4A8` | Metallic 1.0, roughness 0.2 on the belly, 0.45 at the foot; hammer-mark normal at 6 mm. |
| Parcel gilt | `#C9A227` | Rim, finial, foot ring, spout sleeve, handle boss: metallic, roughness 0.3, rubbed silver at edges. |
| Tarnish | `#5A5850` | Dark film in the lower third and crevices, AO-driven. |
| Niello | `#2A2622` | Black inlay in the chased band. |
| Woad enamel | `#3E5470` | Glassy enamel in the medallion, roughness 0.1. |

**Grab points.** One hand on the handle, or under one arm by the neck; thrown, it tumbles handle-over.

**When it breaks.** Unbreakable (fragility 999). Dents instead: shape-key state 1 above 5 m/s (two dents on the belly), state 2 above 9 m/s (crushed shoulder, lid bent open). Worth is unchanged; the lid never detaches. Every impact is loud — a bell-like clang that is a noise event.

**`LootItem` asset values**

`Worth = 180`, `Bulk = 2`, `Fragility = 999`, `IsArtifact = false`, `DisplayName = "Silver Ewer"`

---

<a id="illuminated-psalter"></a>
### Illuminated Psalter

![Illuminated Psalter concept sheet](concept/high/illuminated-psalter.png)

*A painted psalter in a jewelled treasure binding: ivory, gilt and cabochons on the cover, a hundred and eighty leaves inside.*

| Worth | Bulk | Fragility | Carry | Size | Budget | Concept |
|---|---|---|---|---|---|---|
| 350 coin | 1 st | shatters above 5 m/s impact | one player | 0.19 × 0.26 × 0.08 m (W × D × H, closed, lying flat) | ≤ 1.5k tris, 1024² set; 1 leaf card mesh (2 tris) with a 256² illuminated-page atlas of 4 variants. | [SVG](concept/high/illuminated-psalter.svg) · [PNG](concept/high/illuminated-psalter.png) |

A book of the psalms, painted by men who spent a year on it, bound by men who spent another year making sure you would want to steal it. The cover is a carved ivory panel in a gilt frame studded with garnets and sapphires; the leaves inside are gilded and coloured. Keep it dry, keep it away from IGNIS, and do not drop it — for if it lands hard, the pages go everywhere, and the pages are where the worth was.
...
| Material | Hex | Notes |
|---|---|---|
| Gilt (orpiment) | `#C9A227` | Frame, catches, page edges: metallic, roughness 0.3. |
| Ivory | `#D8CDB0` | Carved plaque: subsurface-ish warm, roughness 0.45, dirt in the carved lines. |
| Tawed leather | `#5E3A2A` | Boards and straps: roughness 0.7, cracked hinges, darker at the edges. |
| Vellum | `#DCD2BA` | Page edges and the loose-leaf sprites: matte 0.85. |
| Garnet | `#6E1F2A` | Cabochons: glossy 0.1. |
| Sapphire | `#3E5470` | Cabochons: glossy 0.1 (a woad-blue stone, not lapis). |

**Grab points.** Two hands on the spine and fore-edge, or tucked under one arm (it rides flat against the body); thrown, it spins flat.

**When it breaks.** Above 5 m/s the spine hinge splits: 3 pieces — the jewelled front board (keeps 150 coin), the back board with the text block (0), and 24 loose illuminated leaves that scatter as flat physics cards (0 each). Fire (IGNIS or a candle, 4 s contact) or water (dropped in the moat, a font, a spilled ewer) ruins the text block in place: worth falls to 150 without breaking.

**`LootItem` asset values**

`Worth = 350`, `Bulk = 1`, `Fragility = 5`, `IsArtifact = false`, `DisplayName = "Illuminated Psalter"`
...
![Coin Coffer concept sheet](concept/high/coin-coffer.png)

*An iron-banded oak strongbox full of silver pennies in linen bags; heavy, sturdy, and it bursts spectacularly.*

| Worth | Bulk | Fragility | Carry | Size | Budget | Concept |
|---|---|---|---|---|---|---|
| 450 coin | 8 st | shatters above 8 m/s impact | one player | 0.50 × 0.32 × 0.30 m (W × D × H) | ≤ 3k tris (box + lid + bands 2.2k, padlock 0.3k, handles 0.3k, bag 0.2k; coins instanced), 1024² set. | [SVG](concept/high/coin-coffer.svg) · [PNG](concept/high/coin-coffer.png) |

The lord's ready money: four linen bags of silver pennies in an oak chest bound with more iron than a prison door, with a padlock the size of a fist. It weighs eight stone, so one of you can carry it, slowly, and complain about it the whole way. It is sturdy — but drop it from the wall-walk and the lid comes off, the bags split, and four hundred pennies go skittering across the flagstones like a hailstorm announcing your exact position.
...
| Material | Hex | Notes |
|---|---|---|
| Oak | `#6B4E33` | Planks: straight grain along the length, roughness 0.75, darker end grain; tiling 0.5 m on a trim. |
| Strap iron | `#3A3632` | Blackened, roughness 0.55, nail heads brighter. |
| Rust | `#7A4A2A` | Blooms around nails and along strap edges (mask), not madder. |
| Silver penny | `#B8B4A8` | Instanced coins: metallic, roughness 0.35, 1 atlas tile. |
| Linen bag | `#C4B89C` | Bags inside: cloth normal, tied necks; split variant. |

**Grab points.** Both drop handles (one carrier, 8 st: heavy-carry walk at 70 % speed); or one hand on each end corner. Two players may share it to carry at full speed.

**When it breaks.** Sturdy to 8 m/s. Above that it bursts: 3 pieces (lid, carcass, padlock + hasp) and the 4 bags split, spilling 40 penny-stack pickups of 10 coin each (400) across a 3 m fan; the empty carcass keeps 50 as oak and iron. The spill is a loud noise event (like a lancet).

**`LootItem` asset values**

`Worth = 450`, `Bulk = 8`, `Fragility = 8`, `IsArtifact = false`, `DisplayName = "Coin Coffer"`

---

<a id="gilded-altarpiece"></a>
### Gilded Altarpiece — artifact

![Gilded Altarpiece concept sheet](concept/high/gilded-altarpiece.png)

*The chapel's painted and gilded triptych — fourteen stone, two carriers, and it dies on the first stair.*

| Worth | Bulk | Fragility | Carry | Size | Budget | Concept |
|---|---|---|---|---|---|---|
| 1400 coin | 14 st | shatters above 3 m/s impact | two players | 1.10 × 0.20 × 1.60 m closed (W × D × H); 2.20 m wide open | ≤ 5k tris (dual-carry), 1024² set + a 1024² painted-panel atlas for the four painted faces. | [SVG](concept/high/gilded-altarpiece.svg) · [PNG](concept/high/gilded-altarpiece.png) |

And here it is, the one the pitch promised you: a triptych of painted saints on a ground of beaten gold, a gabled centre panel with the Virgin enthroned, two wings that fold shut, and a predella of little roundels along the bottom. It is fourteen stone, it wants two of you, and it will not fit through the window you came in by. The instant either of you lets go on the stairs it goes down them, and ceases to be worth anything at all.
...
| Material | Hex | Notes |
|---|---|---|
| Gilt (orpiment) | `#C9A227` | Gold-leaf ground: metallic 1.0, roughness 0.25, punch-work normal; rubbed to bole (#7E2A26 at 40 %) at edges. |
| Woad azure | `#3E5470` | Painted mantles, roundels: matte tempera 0.8. |
| Kermes | `#7E2A26` | Painted robes and the bole under the gilt. |
| Gesso | `#DCD2BA` | Faces, the Child, emblem: chalky 0.9. |
| Oak panel | `#5E4630` | Frames, backs, predella box, wood at chips; worm-hole decals on the back. |
| Grisaille | `#8A8274` | Outer faces of the wings: grey monochrome painted figures. |

**Grab points.** Two carriers: carrier A at the left end of the predella, carrier B at the right (the iron carrying staples); a single player can only drag it (it scrapes, loud, and takes 1 m/s impacts). Carried closed.

**When it breaks.** Fragility 3 m/s: any drop from carry height, any stair tumble, any hard doorframe clip. It breaks into 5 pieces — two wings (torn at the hinges), the centre panel split along its two plank joints (3 boards), the predella, and the gable pinnacle — and every piece is worth 0: gilt kindling. No coin spills.

**`LootItem` asset values**

`Worth = 1400`, `Bulk = 14`, `Fragility = 3`, `IsArtifact = true`, `DisplayName = "Gilded Altarpiece"`
